...
class Play:
    ...
    def __init__(self, how_many):
        ...
        # Store original button colours so they can be reset each round
        self.button_colour_list = ["#FF69B4", "#1565C0", "#90EE90", "#CC6600"]
        ...
        # Layout matches screenshot: heading | score | country | hint | results | buttons
        play_labels_list = [
            ["Round # of #", ("Arial", "16", "bold"), None, 0],
            ["Score: 0", body_font, "#FFF2CC", 1],
            ["Country Displays Here", body_font, "#90EE90", 2],
            ["", body_font, None, 3],
            ["Results Go Here", body_font, "#D5E8D4", 4]
        ]
        ...
        self.country_label = play_labels_ref[2]
        self.hint_label = play_labels_ref[3]
        ...
        for item in range(0, 4):
            make_answer_button = Button(self.answer_frame, font=("Arial", 12),
                                        ...
                                        bg=self.button_colour_list[item],
    ...
    def new_round(self):
        ...
        # Update answer buttons with shuffled options, re-enable and reset colours
        for count, item in enumerate(self.answer_button_ref):
            item.config(text=self.current_options[count], state=NORMAL,
                        bg=self.button_colour_list[count], fg="#FFFFFF")
    ...
    def round_results(self, user_choice):
        ...
        country_fact = self.current_question[2] if len(self.current_question) > 2 else "No fact available."
        ...
        if chosen_answer == correct_answer:
            ...
            self.answer_button_ref[user_choice].config(bg="#228B22")
            ...
            # Highlight the clicked button in red and correct answer in green
            self.answer_button_ref[user_choice].config(bg="#CC0000")
            for count, item in enumerate(self.answer_button_ref):
    ...
    def to_hints(self):
        ...
        # Update hint label with background colour only when message is showing
